...
const TabContent = ({ content, t }) => {
  ...
  const handleMergedView = useCallback(() => {
    ...
    try {
      ...
      // Merge content and reasoning_content from all deltas
      let role = '';
      let contentParts = [];
      let reasoningParts = [];
      let toolCalls = {};
      ...
      setDisplayContent(JSON.stringify(merged, null, 2));
      setMergedView(true);
      setFormatted(false);
    } catch {
      Toast.warning(t('非有效JSON'));
      ...
      {loading ? (
        ...
        <Tabs>
          ...
          <TabPane tab={t('上游响应')} itemKey='upstream_response'>
            <TabContent content={detail.upstream_response} t={t} />
          </TabPane>
          <TabPane tab={t('下游响应')} itemKey='downstream_response'>
            <TabContent content={detail.downstream_response} t={t} />
          </TabPane>
        </Tabs>
      ) : (
